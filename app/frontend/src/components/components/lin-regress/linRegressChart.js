...
export class LinRegressChart extends Component {
	...
	updateScales() {
		const allPoints = this.props.points.concat(this.props.bestFitLine);
	...
	updateAxes() {
		let xAxisFunction = d3.axisBottom().scale(this.xScale).ticks(5, "s");

		let yAxisFunction = d3.axisLeft().scale(this.yScale).ticks(5, "s");
			...
			<div className="lin-regress__chart">
				...
				>
					...
					<g
						...
						transform={`translate(${this.state.margin.left}, ${
